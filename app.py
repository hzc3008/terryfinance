...
def index():
    ...
    fifo AS (SELECT
    	c.symbol, c.sumshares, t.shares, t.price, t.timestamp,
    	c.sumshares - SUM(t.shares) OVER (PARTITION BY t.symbol ORDER BY t.timestamp DESC) AS cum_shares

    FROM
    	currentstock AS c
    LEFT JOIN transactions AS t
    ON t.id = c.id AND t.symbol = c.symbol
    WHERE
    	t.shares > 0),

    fifoshares AS (SELECT
    	f.*,
    	CASE WHEN f.cum_shares >=0 THEN f.shares
    	ELSE shares + cum_shares END AS usedshares
    FROM
    	fifo AS f
    WHERE
    	CASE WHEN f.cum_shares >=0 THEN f.shares
    	ELSE shares + cum_shares END > 0),

    	fifounit AS(
    SELECT
    	fs.symbol, SUM(fs.price * fs.usedshares) / SUM(fs.usedshares) AS fifounitcost
    FROM
    	fifoshares AS fs
    GROUP BY
    	fs.symbol)

    SELECT * FROM currentstock AS c JOIN fifounit AS f On c.symbol = f.symbol
    """, session["user_id"])
    ...
    for stock in userstocks:
        ...
        DailyChange = Quotepricedictionary["change"]
        ChangePercent = Quotepricedictionary["changePercent"]
        # Write the current price into the stock dictionary as key "price" = value "currentprice"
        stock["price"] = currentprice
        stock["change"] = DailyChange
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Chek if this guy/girl has sufficient money
        ...
        db.execute("INSERT INTO transactions(id, symbol, shares, price) VALUES (?, ?, ?, ?);", session["user_id"], price["symbol"], shares, price["price"])
